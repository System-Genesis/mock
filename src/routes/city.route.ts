...
router.get("/personalNumber/:personalNumber", (req: Request, res: Response) => {
  ...
  if (result) {
    ...
    res
      .status(400)
      .send(
        `person with personalNumber: ${personalNumber} isn't exists in city`
      );
  }
});
...
router.get("/identityCard/:identityCard", (req: Request, res: Response) => {
  ...
  if (result) {
    ...
    res
      .status(400)
      .send(`person with identityCard: ${identityCard} isn't exists in city`);
  }
});
...
router.get("/domainUser/:domainUser", (req: Request, res: Response) => {
  ...
  if (result) {
    ...
    res
      .status(400)
      .send(`person with domainUser: ${domainUser} isn't exists in city`);
  }
});
